...
def setup_plot(ax, data, std_data, synergistic_captions, redundant_captions, name):
    ...
    ax.bar(
        ...
        width,
        yerr=np.divide([i[0] for i in std_data], math.sqrt(5)),#std_data[:, 0] / math.sqrt(5),
        label='MI',
        color='#ff4b4b',
        alpha=0.9,
        zorder=3,
    ...
    ax.bar(
        ...
        width,
        yerr=np.divide([i[1] for i in std_data], math.sqrt(5)),#std_data[:, 1] / math.sqrt(5),
    ...
    for i, red_values in enumerate([i[2] for i in data]):
        ...
        for j, val in enumerate(red_values):
            ...
            bar = ax.bar(
                ...
                val,
                width,
                yerr=np.divide(std_vals[j], math.sqrt(5)),
